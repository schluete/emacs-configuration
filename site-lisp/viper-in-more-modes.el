...
(defcustom viper-imm-ibuffer-bindings t
  "Ibuffer bindings."
  :type  'boolean
  :group 'viper-in-more-modes)

(defvar viper-imm-ibuffer-mode-vi-map
  (let ((map (make-sparse-keymap)))
    (define-key map "t" 'ibuffer-toggle-marks)
    (define-key map " " 'ibuffer-mark-forward)
    (define-key map "\r" 'ibuffer-visit-buffer)
    (define-key map "m" 'ibuffer-mark-forward)
    (define-key map "u" 'ibuffer-unmark-forward)
    (define-key map "g" 'ibuffer-update)
    (define-key map "d" 'ibuffer-do-delete)
    (define-key map "q" 'ibuffer-quit)
    (define-key map "S" 'ibuffer-do-save)
    (define-key map "Q" 'ibuffer-do-query-replace)
    (define-key map "K" 'ibuffer-do-kill-lines)
    (define-key map "//" 'ibuffer-switch-to-saved-filters)
    (define-key map "/n" 'ibuffer-filter-by-name)
    (define-key map "/a" 'ibuffer-add-saved-filters)
    (define-key map "/s" 'ibuffer-save-filters)
    (define-key map "/p" 'ibuffer-pop-filter)
    map))

(when viper-imm-ibuffer-bindings
  (viper-modify-major-mode 'ibuffer-mode
                           'vi-state
